        ...
        {selectedContact ? (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-emerald-50/20 dark:bg-zinc-950/20">
              ...
              {(conversations[selectedContact.id] ?? []).map(messageItem => (
                <div
                  key={messageItem.id}
                  className={`flex items-start gap-4 ${messageItem.from === 'me' ? 'flex-row-reverse ml-12' : 'mr-12'}`}
                >
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shrink-0 ${messageItem.from === 'me' ? 'bg-emerald-600 text-white' : 'bg-emerald-100 dark:bg-emerald-950/30 text-emerald-600'}`}>
                    {messageItem.from === 'me' ? 'A' : selectedContact.name.charAt(0)}
                  </div>
                  <div className={`p-4 rounded-2xl shadow-sm ${messageItem.from === 'me' ? 'bg-emerald-600 text-white rounded-tr-none shadow-lg shadow-emerald-100 dark:shadow-none' : 'bg-white dark:bg-zinc-800 border border-emerald-100 dark:border-zinc-800 rounded-tl-none'}`}>
                    <p className="text-sm leading-6">{messageItem.text}</p>
                    <span className={`text-[10px] mt-2 block ${messageItem.from === 'me' ? 'text-emerald-100' : 'text-zinc-400 dark:text-zinc-500'}`}>{messageItem.time}</span>
                  </div>
                </div>
              ))}
            </div>
            <div className="p-6 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border-t border-emerald-50 dark:border-zinc-800">
               <div className="relative flex items-center gap-3">
                 <button className="p-2.5 text-emerald-600 hover:bg-emerald-50 rounded-xl">
                   <Paperclip size={20} />
                 </button>
                 <div className="flex-1 relative">
                   <input 
                     ...
                     className="w-full bg-emerald-50/50 dark:bg-zinc-800 border-emerald-100 dark:border-zinc-700 rounded-2xl py-3.5 pl-4 pr-12 focus:ring-2 focus:ring-emerald-500 focus:bg-white dark:focus:bg-zinc-900 transition-all text-sm"
                   />
                   <button className="absolute right-3 top-1/2 -translate-y-1/2 text-emerald-600 p-1.5 hover:bg-emerald-100/50 rounded-lg">
                     <Smile size={18} />
                   </button>
                 </div>
                 <button
                   onClick={handleSendMessage}
                   className="p-3.5 bg-emerald-600 text-white rounded-2xl shadow-lg shadow-emerald-200 transition-all hover:scale-105 active:scale-95"
                 >
                   <Send size={20} />
                 </button>
               </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
            <div className="w-24 h-24 bg-emerald-50 rounded-full flex items-center justify-center text-emerald-200 mb-6">
              <MessageCircle size={48} />
            </div>
            <h3 className="text-2xl font-bold text-emerald-950">Select a candidate</h3>
            <p className="text-zinc-500 max-w-sm mt-2">Pick a candidate from the left panel to review or send approval/rejection messages.</p>
          </div>
        )}
      </div>
    </div>
